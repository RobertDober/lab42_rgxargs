...
RSpec.describe "README.md" do
  ...
  context "Setup for speculations" do
    ...
    context "And What About Posix?" do
      ...
      it "I can parse posix style options (README.md:63)" do
        kwds, positionals, _errors = parser.parse(%w{-xy --a=42 --hello=b hello})
        expect(kwds).to eq(os(x:true, y:true, a: "42", hello: "b"))
        expect(positionals).to eq(%w{hello})
      end
      it "we can use `--` to get positionals with leading `-`s and we also accept long flags (therefore the = is needed for values) (README.md:70)" do
    ...
    context "Conversion Of Keyword Parameters" do
      ...
      it "the parsed value for the `lower` argument will be an `Integer`, while the other parsed values remain `Strings` (README.md:88)" do
        expect(parser.parse(%w[lower: 42 upper: 43]).first)
        .to eq(os(lower: 42, upper: "43"))
      end
      # README.md:93
      context "Withe predefined matchers" do
        it "such common converters are predefined of course, and thusly (README.md:96)" do
          parser.add_conversion(:alpha, :int)
          expect(parser.parse(%w[alpha: 42]).first)
          .to eq(os(alpha: 42))
        end
        it "you can see all predefined matchers as follows (README.md:103)" do
          predefined_matchers =
          %w[ existing_dirs int int_list int_range list range ]
          .join("\n\t")
          expect(parser.predefined_matchers).to eq(predefined_matchers)
        end
        it "We can also just pass in the converter without a guard (README.md:111)" do
          parser.add_conversion(:maybe_int, &:to_i)
          expect(parser.parse(%w[maybe_int: fourtytwo]).first)
          .to eq(os(maybe_int: 0))
        end
        it "converters with guards do return meaningful error messages (README.md:119)" do
    ...
    context "General Syntax" do
      ...
      it "we have some predefined syntaxes, of course (README.md:138)" do
        parser.add_syntax(:range)
        _, my_range, _ = parser.parse(%w{1..3})
        expect(my_range.first).to eq(1..3)
      end
      it "they are of course applied to **all** arguments, e.g. (README.md:145)" do
        parser.add_syntax(:range)
      ...
      it "there is a special `int_list` converter available (README.md:157)" do
        parser.add_syntax(:int_list)
        _, list, _ = parser.parse(%w{1,2,4})
        expect(list.first).to eq([1,2,4])
      end
      it "Of course a `add_syntax` (for positionals) and  `add_conversion` (for keywords) can be mixed using the same converters under the hood (README.md:164)" do
    ...
    context "Giving Names to Syntaxes" do
      it "therefore we have (README.md:183)" do
      ...
      it "for more complex possibilities of timestamps one can use a little DSL (README.md:192)" do
    ...
    context "Constraints" do
      ...
      context "Allowing Keyword Params" do
        it "Allowing keywords means, all others are forbidden (README.md:208)" do
          parser.allow_kwds(:version)

          _, _, errors = parser.parse(%w[vision: 41])
          expect(errors) == [[:unauthorized_kwd, :vision]]
        end
        it "the allowed work as expected (README.md:216)" do
          parser.allow_kwds(:version)
    ...
    context "Require Keyword Params" do
      it "if required keywords are absent... (README.md:227)" do
      ...
      it "if they are present... (README.md:239)" do
        parser.require_kwds(:from)
        parser.add_conversion(:to, :int, :required)
    ...
    context "Syntactic Sugar" do
      ...
      it "the conversion works of course as expected (README.md:263)" do
      ...
      context "Using predefined matches in the DSL" do
        ...
        it "we can parse the keyword arguments with existing dirs w/o an error (README.md:280)" do
