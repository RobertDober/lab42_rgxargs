...
RSpec.describe "README.md" do
  ...
  context "Setup for speculations" do
    ...
    context "No Config Out Of The Box" do
      it "I can parse ruby syntax based arguments (README.md:39)" do
        kwds, positionals, _errors = parser.parse(%w{a: 42 hello :b})
        expect(kwds).to eq(os(a: "42", b: true))
        expect(positionals).to eq(%w{hello})
      end
      it "the only error one can get with this null configuration is a missing value for trailing keyword arg (README.md:46)" do
    ...
    context "Conversion Of Keyword Parameters" do
      ...
      it "the parsed value for the `lower` argument will be an `Integer`, while the other parsed values remain `Strings` (README.md:64)" do
        expect(parser.parse(%w[lower: 42 upper: 43]).first)
        .to eq(os(lower: 42, upper: "43"))
      end
      # README.md:69
      context "Withe predefined matchers" do
        it "such common converters are predefined of course, and thusly (README.md:72)" do
          parser.add_conversion(:alpha, :int)
          expect(parser.parse(%w[alpha: 42]).first)
          .to eq(os(alpha: 42))
        end
        it "you can see all predefined matchers as follows (README.md:79)" do
          predefined_matchers = %w[ int int_list int_range list range ].join("\n\t")
          expect(parser.predefined_matchers).to eq(predefined_matchers)
        end
        it "We can also just pass in the converter without a guard (README.md:85)" do
          parser.add_conversion(:maybe_int, &:to_i)
          expect(parser.parse(%w[maybe_int: fourtytwo]).first)
          .to eq(os(maybe_int: 0))
        end
        it "converters with guards do return meaningful error messages (README.md:93)" do
    ...
    context "General Syntax" do
      ...
      it "we have some predefined syntaxes, of course (README.md:112)" do
        parser.add_syntax(:range)
        _, my_range, _ = parser.parse(%w{1..3})
        expect(my_range.first).to eq(1..3)
      end
      it "they are of course applied to **all** arguments, e.g. (README.md:119)" do
        parser.add_syntax(:range)
      ...
      it "there is a special `int_list` converter available (README.md:131)" do
        parser.add_syntax(:int_list)
        _, list, _ = parser.parse(%w{1,2,4})
        expect(list.first).to eq([1,2,4])
      end
      it "Of course a `add_syntax` (for positionals) and  `add_conversion` (for keywords) can be mixed using the same converters under the hood (README.md:138)" do
    ...
    context "Giving Names to Syntaxes" do
      it "therefore we have (README.md:157)" do
      ...
      it "for more complex possibilities of timestamps one can use a little DSL (README.md:166)" do
    ...
    context "Constraints" do
      ...
      context "Allowing Keyword Params" do
        it "Allowing keywords means, all others are forbidden (README.md:182)" do
          parser.allow_kwds(:version)

          _, _, errors = parser.parse(%w[vision: 41])
          expect(errors) == [[:unauthorized_kwd, :vision]]
        end
        it "the allowed work as expected (README.md:190)" do
          parser.allow_kwds(:version)
    ...
    context "Require Keyword Params" do
      it "if required keywords are absent... (README.md:201)" do
      ...
      it "if they are present... (README.md:213)" do
        parser.require_kwds(:from)
        parser.add_conversion(:to, :int, :required)
    ...
    context "Syntactic Sugar" do
      ...
      it "the conversion works of course as expected (README.md:237)" do
